...
function IntroCar({brand, modelNum}){
    // 전달 속성값: brnad - 자동차 분류명, modelNum - 배열순번
    // props는 컴포넌트 호출시 속성값을 객체로 전달하는것을 받아주는 변수다.(변수명은 자유)
    // 사용법: props.속성명
    // 주의: 속성명은 컴포넌트를 호출하는 곳에서 정해진 이름을 똑같이 사용해야함
    // 명시적으로 변수명을 사용히려면 구조분해 할당형식을 사용하면된다.
    // {변수명,변수명,...}
    // 구조분해할당으로 받을 경우의 장점은 
    // 특정 속성명을 지정하는 효과가 있고
    // 몇개의 어떤 속석을 받는지 개발자가 명시적으로 알 수 있다.
    ...
    return(
        <React.Fragment>
            ...
            {/* 추가질문 컴포넌트 호출 */}
            <AskMoreInfo model={setInfo.model} color={setInfo.color} opt={setInfo.opt} />
        </React.Fragment>
    );
...
function DetailCarInfo({model,color,opt}){
    ...
    return(
        <React.Fragment>
            ...
            style={opt} 
            />
            {/* 리액트 style 속성의 값으로 객체를 CSS속성에 맞게
            주면 인라인코드로 CSS를 셋팅할 수 있다. */}
        </React.Fragment>
    );
} /////DetailCarInfo
