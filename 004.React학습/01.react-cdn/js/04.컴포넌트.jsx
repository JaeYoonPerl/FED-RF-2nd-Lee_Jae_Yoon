...
import Avengers from "./avengers.ㅔ룬";
...
// React.Component 컴포넌트 기능 구현 부모 클래스
...
// [ 이미지 생성 공통 컴포넌트 ]
// -> 비구조화할당(구조분해할당)을 사용하여 전달객체를 개별 변수값으로 처리할 수 있다.
...
function FavoriteThings(조아) {
    return (
        <h2>
            내가 좋아하는 색은 {조아.color}이야
            <br />
            그리고 좋아하는 음식은 {조아.food}야!
            <br />
            취미는 {조아.hobby}야! 알겠니???
        </h2>
    );
} //////// FavoriteThiings 컴포넌트

// 좋아하는 색과 음식, 취미를 각각 속성명으로 생성하여 
// 컴포넌트를 호출하면 개별적으로 속성을 구분할 수 있다.
// 출력: 세번째 .root
ReactDOM.render(<FavoriteThings color="와인색" food="와인이" hobby="와인마시기"/>, target[2]);
// 출력: 네번째 .root
ReactDOM.render(<FavoriteThings color="푸르딩딩한색" food="김치전이" hobby="야구보기"/>, target[3]);
