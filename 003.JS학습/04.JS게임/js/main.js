...
// -> 브라우저 지원버전이 너무최신이라 지금은 assert를 사용하자
...
let autoI;
function goR1(){
    // 호출이 한번만 되도록 autoI가 할당전엔 undefined이므로
    // if문에서 false 처리됨! 이것을 이용하자
    if (!autoI){ // false일때만 들어감(할당전에만)
        console.log('토끼 인버발',level.value);
        autoI = setInterval(() => {
            // 토끼 위치이동(1px씩)
            r1.style.left = ++r1pos + 'px';
            // 승자 판별 함수 호출(인터벌내 계속호출)
            whoWinner();
        }, level.value);
