...
/****************************************************** 
                1. 함수명: changeImg
                2. 기능: 버튼별 스틸컷 변경
                3. 원리 : 클릭괸 버튼 텍스트를 읽어 구분하여 if문을 작성후 변경할 이미지
                경로를 만들어서 실제 이미지 src에 할당하여 이미지를 변경한다,
******************************************************/
function changeImg() {
    // 호출된 요소자신은 this키워드로 처리한다.

    // 0. this 키워드를 활용하여 전달된 요소의 버튼 텍스트 읽어오기(구분하기 위해)
    var btnTxt = this.innerText;
    // innerText를 이퀄 오른쪽에 쓰면 요소의 문자데이터를 읽어옴
    // 반면 innerText를 왼쪽에 쓰고 오른쪽에 값을 할달하면 값설정이 됨
    // innerHTML도 마찬가지임(태그처리만 다름)

    // 1. 함수호출 확인
    console.log("나야아!",btnTxt);
