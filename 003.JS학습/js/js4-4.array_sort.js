...
// 예
const arrNumber2 = [380,1000,245,2578];
// 문자값 배열
...
const showImgNum = (arrObj) => { // arrObj 전달된 배열
    ...
    showNum.innerHTML=arrObj.map(v=>`<img src="./images/num/num_0${v}.png" alt="숫자${v}이미지">`).join('');
...
-> join('')빈문자열 결합을 사용하면 배열값이 깨끗하게 
문자열 덩어리로 그대로 출력된다
->>> 배열.map().join('') -> 배열 맵쪼잉
-> join()을 안쓰면 배열의 기본값이 콤마로 연결되어 할당되는데
콤마를 없애려면 반드시 join()을 써야함 
*******************************************************/

console.log('원본 배열:',arrNumber2);
console.log('원본 배열로 태그 달성:',arrNumber2.map(v=>`<숫자>${v}</숫자>`));

console.log('원본 배열로 태그작성한 배열을 문자열로 변경하기:',arrNumber2.map(v=>`<숫자>${v}</숫자>`).join(''));

console.log('원본 배열로 데이터 달성:',arrNumber2.map((v,i)=>`👓회원번호${i+1}:${v}포인트`));
...
function changeSort(e,arrObj) { 
    ...
    // 2-1. 오름차순 : 값 1
    if(optVal == 1){
        // sort() 빼기 연산처리 : 앞수-뒷수(양수결과일경우 순서바꾸기함)
        arrObj.sort((a,b)=>a-b);
    }/// if
    // 2-2. 내림차순 : 값 2
    else if(optVal == 2){
          // sort() 빼기 연산처리 : 뒷수-앞수(양수결과일경우 순서바꾸기함)
          arrObj.sort((a,b)=>b-a);
    }/// else if

    // ((주의)) 원본배열을 정렬후엔 원본배열은 없어진다.
    // 원본배열을 다른 변수에 할당후 다른변수를 정렬후엔
    // 여전히 원본배열은 없어진다.
