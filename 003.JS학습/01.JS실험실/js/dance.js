...
function moveSlide(){
    ...
    // 2. 이동할 타겟박스 left값으로 부모 바운딩top 값 넣기
    // (1) 바운딩 top값이 0 초과일때 처음위치 재설정하기
    if(bTop>0){
        target.style.left = bTop + '0px';
    }
    // (2) 바운딩 top값이 0 이하 -3000이상 일때 부모 바운딩top값으로 위치이동하기
...
function upDownFn(ele,idx){ //// ele - 각 li요소 / idx - 각 li 순번
    ...
    // 스티키 박스 하위 li가 보이는화면 left 기준선에 대한
    // 위치값 바운딩 left값을 이용하여 자기위치에서 위/아래로 이동하는 
    // 수치 변경하기
