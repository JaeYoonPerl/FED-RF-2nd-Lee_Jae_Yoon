...
// 1. 함수이름을 대문자로 시작한다.,
// 2. 호출시 new 키워드를 ㅅ하용하여 인스턴스를 생성한다
// -> 일반함수화 차이점은 개별적인 객체 인스턴스로 함수가 
// 개별화되기 때문에 운용상 독립적 프로세스를 확보하게 된다.
// -> this 키워드를 사용하는 경우는 new로 생성된 인스턴스 객체에서 함수 내부에 있는 
// 변수나 함후를 직접 호출해야할 경우 사용한다.
...
export default function AutoScrollFn() {
    ...
    /********************************************** 
    [ 자동스크롤 기능정의 ]
    1. 스크롤바가 없는 상태에서 또는 스크롤 기능을 막아놓은 상태에서
    마우스 휠 작동시
    아래와 같이 기능구현됨
    (1) 휠 내림 : 다음페이지로 이동
    (2) 휠 올림 : 이전페이지로 이동
    ...
    (주의 : wheel이벤트는 scroll이벤트와는 다름! 마우스의 바퀴를 돌릴때 발생함!)
**********************************************/
    ...
    /* 
    [ window / document / body 세가지는 기본막기불가 설정되어있음 ]
    -> 이벤트 등록시 패시브 모드가 true로 설정됨
    셋팅방법: 
    요소.addEventListener(이벤트명, 함수,{passive:true})
    -> 기존엔 passive:false였는데 이것의 기본값이 true로 변경됨. 의미는 기본기능막기
    못하게 설정됨
...
*/
    ...
    // [ 새로고침시 스크롤바 위치 인덱싱이 되므로 맨위로 강제 이동하기 설정 ]
    // scrollTo(x축이동,y축이동)
    setTimeout(() => {
        window.scrollTo(0, 0);
    }, 500);
...
***************************************/
    ...
    /****************************************** 
        함수명 : movePage
        기능: DT나 Mobilde 모두 페이지 이동시 호출하여
...
function touchEndFn(e){
    ...
    // 읽어와야 하므로 chnangedTouches[0]ㄹㄹ 사용해야함

    // 2. 처음 터치 위치화 마지막 위치의 차 구하기
    let diffValue = mPosEnd - mPosStart;
    /// 음수가 아랫방향이동으로 맞추기 위해 (끝값 - 첫값)
