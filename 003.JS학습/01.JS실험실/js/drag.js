...
function setDrag(clsName) {
    ...
    // 2. 드래그 함수 호출
    // HTML 컬랙션이므로 forEach메서드로 호출
    // forEach((요소, 순번, 전체)=>{})
    ele.forEach((x,y,z) => goDrag(x,z));
    // z는 전체 요소집합 컬렉션임(z-index초기화로 필요함!)
    // goDrag(ele);
} ///// setDrag 함수 //////////////
...
function goDrag(ele, coll) {
    ...
    let lastX = 0,
        lastY = 0;
    //-> 중첩된 최종위치가 처음에는 계산되지 않았으므로 출발 위치인 0값으로
    // 초기값을 넣어준다
    // 초기값을 안넣으면 최초에 앖을 더할때 에러가 발생한다.

    //(4) 움직일때 위치 포인트 : move x, move y
    let moveX, moveY;
    // (5) 위치이종 차이 계산 결과 변수 : result x, result y
    let resultX, resultY;

    // 3. 함수 만들기
    // 할당형 함수를 만들경우 이벤트 설정보다 위에서 만들어 준다.
    ...
    const dMove = (e) => {
        ...
        if (dragSts) {
            ...
            // - 브라우저용 포인터 위치는 pageX,pageY를 사용한다.
            moveX = e.pageX;
            moveY = e.pageY;
        ...
        // 드래그 중일때는 주먹손(grabbing), 드래그아닐때 편손(grab)
