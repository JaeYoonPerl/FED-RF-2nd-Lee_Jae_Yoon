...
export default function TopArea({ changeMenu }) {
    ...
    // 메뉴 배열
    const menuArr = ["FASHION", "BEAUTY", "LIVING", "PEOPLE", "VIDEO", "RUNWAY", "TIME & GEM", "SHOPPING"];
            ...
            <header className="top-area ibx common-area">
                ...
                <nav className="gnb">
                    <ul>
                        {menuArr.map((v) => (
                            <li>
                                <a href="#" onClick={()=>{chgMenuFn(v)}}>{v}</a>
                            </li>
                        ))}
                        <li>
                            {/* <!-- 돋보기 검색버튼 --> */}
                ...
                {/* <!-- 모바일용 햄버거버튼 --> */}
                {/* <a href="#" className="mobtn hbtn fi fi-nav-icon"><span className="ir">GNB button</span></a> */}
                {/* <!-- 모바일용 검색버튼 --> */}
                {/* <a href="#" className="mobtn sbtn fi fi-search"><span className="ir">search</span></a>  */}
            </header>
        </div>
    );
}
