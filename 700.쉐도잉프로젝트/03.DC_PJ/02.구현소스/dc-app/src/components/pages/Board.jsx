...
// 로컬 스토리지 게시판 기본 데이터 제이슨 -> 로컬쓰로 대체
// import baseData from "../data/board.json";
// 리액트 웹팩에서 제이슨은 이름을 지어서 불어오면된다.
    ...
    return (
        <main className="cont">
            ...
            }
            {
                ...
                // sts값은 문자열이므로 파싱하여 객체로 보ㅓ냄
                mode == "W" && <WriteMode sts={JSON.parse(sts)} />
            }
            {
                //  4. 수정 모드일 경우 상세보기 출력하기
                // sts값은 문자열이므로 파싱하여 객체로 보ㅓ냄
                mode == "M" && <ModifyMode sts={JSON.parse(sts)} />
            }
                    ...
                    <tr>
                        <td>
                            ...
                            }
                            {
                                ...
                                { // 로그인한 상태이고 글쓴이와 일치 할때
                                  // 수정보드 이동버튼이 노출됨
                                  // 현재글은 selRecord 참조변수에 저장됨
                                  // 글정보 항목중 uid 가 사용자 아이디임
                                  // 로그인 상태정보하위의 sts.uid와 비교함
                                  ( mode == "R" && "sts" && 
                                  JSON.parse(sts).uid == selRecord.current.uid) && <button onClick={clickButton}>Modify</button>
                                }
                                </>
                            }
                            {
...
const ReadMode = ({ selRecord }) => {
    // 읽기 모드가 호출되었다는 것은
    // 리스트의 제목이 클릭되었다는 의미
    // 따라서 현재 레크드 값도 저장되었다는 의미
    //console.log("전달된 참조면수:", selRecord.current);
    // 전달된 데이터 객체를 변수에 할당
