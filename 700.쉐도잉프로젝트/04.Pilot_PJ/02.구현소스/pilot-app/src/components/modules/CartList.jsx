                ...
                <table>
                    ...
                    <tbody>
                        <tr>
                            <td colSpan={8}>
                                {/* 내부 스크롤박스 div */}
                                <div className="scbar" style={{ display: "block", overflowY: "auto", height: "40vh", width: "60vw" }}>
                                    {/* 내부용 스크롤되는 테이블 */}
                                    <table
                                        style={{
                                            margin: "0",
                                        ...
                                        **************************/}
                                            ...
                                            {selData.map((v, i) => (
                                                <tr key={i}>
                                                    ...
                                                    {/* 상품이미지 */}
                                                    <td>
                                                        <img src={process.env.PUBLIC_URL + `/images/goods/${v.cat}/${v.ginfo[0]}.png`} alt="item" />
                                                    </td>
                                                    <td>{v.ginfo[1]}</td>
                                                    <td>{v.ginfo[2]} </td>
                                                    ...
                                                    <td className="cnt-part">
                                                        <div>
                                                            <span>
                                                                <input type="text" className="item-cnt" readOnly="" value={v.cnt} onChange={() => {}} />
                                                    ...
                                                    <td>
                                                        <span className="sum-num1">{addComma(v.ginfo[3] * v.cnt)}</span>원{/* 계산된 합계금액 숫자만 히든필드에 넣어놓고 총합계 계산에 사용함 */}
                                                        <input className="sum-num2" type="hidden" defaultValue={v.ginfo[3] * v.cnt}></input>
                                                    </td>
                                                    <td>
                                                        {/* 데이터삭제 기능 버튼 */}
                                                        <button className="cfn" onClick={()=>{
                                                            // confirm()의 "확인" 클릭시 true
                                                            if(window.confirm("정말정말정말로 지우겠습니까? 할인도 하는데??")){
                                                                // console.log("삭제함");
                                                                // console.log("현재객체",selData);
                                                                // console.log("지울순번",i);
                                                                
                                                                // splice 자체를 찍으면 지워진 요소가 찍힘
                                                                // console.log("지우기",selData.splice(i));
